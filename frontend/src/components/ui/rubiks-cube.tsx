...
const getRandomMaterial = () => MATERIALS[Math.floor(Math.random() * MATERIALS.length)];

function CubeCluster() {
  const groupRef = useRef<THREE.Group>(null);

  // Pre-assign materials so they don't change on re-render
  const cubeMaterials = useMemo(() => POSITIONS.map(() => getRandomMaterial()), []);
  ...
  return (
    <group ref={groupRef}>
      {POSITIONS.map((pos, i) => (
        <mesh key={i} position={pos} material={cubeMaterials[i]} castShadow receiveShadow>
